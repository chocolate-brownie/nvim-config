...
-- [[ 42 School and C Programming Specific Settings ]] --

-- Set tab width to 4 spaces (42 norm requirement)
vim.opt.tabstop = 4
...
-- Column guide for 42's max line length
...
-- Automatically insert header guards in .h files
...
vim.api.nvim_create_autocmd({ "FileType" }, {
	pattern = "c",
	callback = function()
		-- Set make program to use Makefile
		vim.opt_local.makeprg = "make"
		-- Add additional errorformat for norminette
		vim.opt_local.errorformat:append("%f:%l:%c: %m")
	end,
})
...
require("lazy").setup({
	...
	--
	-- 42 Header Generator for Neovim
	{
		"42Paris/42header",
		lazy = false, -- We want this always loaded
		config = function()
			vim.g.user42 = "mgodawat" -- Your 42 login
			vim.g.mail42 = "mgodawat@student.42.fr" -- Your 42 email
			-- Optional: Map the header command to a key
			vim.keymap.set("n", "<leader>h", ":Stdheader<CR>", { silent = true })
		end,
	},
	-- 42 C code formatter/
	{
	...
	},
	{
		...
		config = function()
			...
			local servers = {
				clangd = {
					cmd = {
						...
						"--malloc-trim",
						"--pch-storage=memory",
						-- These help with 42's coding style
					...
					filetypes = { "c", "h" }, -- Focus only on C files for 42
			...
			require("mason-lspconfig").setup({
				ensure_installed = {
					"clangd", -- For C programming at 42
				},
				automatic_installation = true, -- Automatically install new servers
				handlers = {
